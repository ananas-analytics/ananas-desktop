...
public class ExploreCommand implements Callable<Integer> {
  ...
  @CommandLine.Option(
    names = {"-m", "--param"},
    description =
      "Parameter values, the parameter must be defined in ananas file")
  ...
  @CommandLine.Parameters(index = "0", description = "Id of the step to explore, must be source or destination")
  ...
  public Integer call() {
    ...
    paginationBody.params = analyticsBoard.variables.stream().collect(Collectors.toMap(v -> v.name, v -> v));
