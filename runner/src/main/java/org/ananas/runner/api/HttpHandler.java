...
class HttpHandler {
	...
	//TEST
	static Route paginateStep = (Request request, Response response) -> {
		...
		PaginationBody paginationBody =
				body == null || body.length() == 0 ? new PaginationBody() : JsonUtil.fromJson(body,
	...
	//TEST
	static Route testDag = (Request request, Response response) -> {
	...
	//PREDICT
	static Route runDag = (Request request, Response response) -> {
		String id = request.params(":projectid");
	...
	static Route cancelPipeline = (Request request, Response response) -> {
		String id = request.params(":id");
	...
	static Route listJobs = (Request request, Response response) -> {
		Runner runner = new BeamRunner();
	...
	static Route dataView = (Request request, Response response) -> {
		String tablename = request.params(":tablename");
		if (tablename == null) {
			return JsonUtil.toJson(ApiResponseBuilder.Of().KO(new NoSuchElementException("stepid not found")).build());
		}

		DataViewRepository repository = new DataViewRepository();
		return JsonUtil.toJson(
				ApiResponseBuilder.Of().OK(repository.query(request.queryParams("sql"), tablename)).build());
	};


	static ExceptionHandler error = (Exception e, Request request, Response response) -> {
		response.status(200);
	...
	static Route healtcheck =
			(Request request, Response response) -> JsonUtil.toJson(ApiResponseBuilder.Of().OK(h).build());
