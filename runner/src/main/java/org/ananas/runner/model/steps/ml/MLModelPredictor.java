...
public class MLModelPredictor extends AbstractStepRunner implements StepRunner, Serializable {
	...
	public MLModelPredictor(Step MLStep,
							StepRunner previous) {
		...
		final Predictor model =
				(Predictor) SerializationUtils.deserialize(MLHookTemplate.getSerializedModelPath(this.stepId),
						true);
		...
		for (Schema.Field f : previousSchemaFields) {
			if (f.getName().equalsIgnoreCase(yColumnName)) {
				...
				schemaBuilder.addNullableField(f.getName(),
		...
		//resolves yColIndex Y response variable index in input file if it exists otherwise we add it at last Index
		...
		Schema outputSchema =
				this.isYInput ? schema : MLHookTemplate.getSchemaBuilder(schema).addNullableField(yColumnName,
						model.getPredictedType()).build();
		...
		Combine.Globally<Row, List<Attribute>> globalAttributes = Combine.globally(new CombineAttributeFn());

		Combine.GloballyAsSingletonView<Row, List<Attribute>> t =
				globalAttributes.asSingletonView();

		final PCollectionView<List<Attribute>> attView =
				previous.getOutput().apply(t);

		this.output = previous.getOutput()
				.apply("predict", ParDo.of(
						(new DoFn<Row, Row>() {
							private static final long serialVersionUID = 7683479078488097137L;

							@ProcessElement
							public void processElement(@Element Row input,
	...
	public static class CombineAttributeFn extends Combine.CombineFn<Row, CombineAttributeFn.Accum, List<Attribute>> {
		...
		@Override
		public Coder<CombineAttributeFn.Accum> getAccumulatorCoder(CoderRegistry registry,
	 ...
	 * @return
	 */
	protected double[] rowToVector(List<Schema.Field> schema, Row row, List<Attribute> attributeList) {
