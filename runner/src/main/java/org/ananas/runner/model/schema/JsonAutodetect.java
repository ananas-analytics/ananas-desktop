...
public class JsonAutodetect {
  ...
  public static Schema autodetectBson(
      Iterator<Document> jsons, FlattenMode mode, boolean parseString, Integer limit) {
    Map<String, Schema.FieldType> types = new HashMap<>();
    int i = 0;
    while (jsons.hasNext()) {
  ...
  public static void inferTypes(
      Map<String, Schema.FieldType> types,
      String jsonString,
      FlattenMode mode,
      boolean parseString) {
    try {
  ...
  static void inferTypes(
      Map<String, Schema.FieldType> types, Map<String, Object> o, boolean parseString) {
    for (Map.Entry<String, Object> entry : o.entrySet()) {
      if (types.get(entry.getKey()) != null) {
        types.put(
            entry.getKey(),
            TypeInferer.mergeType(
                types.get(entry.getKey()), TypeInferer.inferType(entry.getValue(), parseString)));
      } else {
