...
public class CSVStepConfig {
	...
	public CSVStepConfig(StepType type, Map<String, Object> config) {
		this.recordSeparator =
				config.get(StepConfig.recordSeparator) ==
						null ? CSVFormat.DEFAULT.getRecordSeparator() : (String) config.get(
						StepConfig.recordSeparator);
		this.delimiter = config.get(StepConfig.DELIMITER) ==
								 null ? CSVFormat.DEFAULT.getDelimiter() : (char) config.get(
				StepConfig.DELIMITER);
