...
public class ParseResultReader extends PTransform<PCollection<ParseResult>, PCollection<Row>> implements Serializable {
				...
				ParDo.of(
						new DoFn<ParseResult, Row>() {
							...
							public void processElement(ProcessContext ctx) {
								...
								if (txt.isSuccess()) {
									ctx.output(Row.withSchema(ParseResultReader.this.schema).addValue(
											txt.getContent()).addValue(toJSON(txt.getMetadata())).build());
								} else {
									ParseResultReader.this.errors.addError(ExceptionHandler.ErrorCode.GENERAL,
											txt.getError().getMessage());
