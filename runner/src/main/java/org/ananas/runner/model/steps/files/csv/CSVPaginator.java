...
public class CSVPaginator extends AbstractFilePaginator implements Paginator {
	...
	public CSVPaginator(String id, CSVStepConfig csvConfig) {
		...
		this.format = CSVFormat.DEFAULT.withDelimiter(this.csvConfig.delimiter).withRecordSeparator(
				this.csvConfig.recordSeparator);
	...
	protected Schema autodetect(Integer pageSize) {
		...
		try {
			...
			throw new RuntimeException("Can't parse CSV file " + this.csvConfig.url + " :" + e.getMessage());
		}
		return inputschema;
	}

	private Schema withoutHeader(Iterator<String> it, CSVFormat format, Map<Integer, String> m) throws IOException {
	...
	public Iterable<Row> iterateRows(Integer page, Integer pageSize) {
		return PageProcessor.readFile(this.url, page, pageSize, (e, i) -> {
			if (page == 0 && this.csvConfig.hasHeader && i == 0) {
				return null;
			}
			Iterator<Row> it = BeamTableUtils.csvLines2BeamRows(this.format, e, this.schema).iterator();
