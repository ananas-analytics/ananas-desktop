...
public class TruncatedTextSource extends FileBasedSource<String> {
	...
	TruncatedTextSource(ValueProvider<String> fileSpec, EmptyMatchTreatment emptyMatchTreatment,
						byte[] delimiter) {
		super(fileSpec, emptyMatchTreatment, 1L);
		this.delimiter = delimiter;
	}

	private TruncatedTextSource(MatchResult.Metadata metadata, long start, long end, byte[] delimiter) {
	...
	protected FileBasedSource<String> createForSubrangeOfFile(
		...
		return new TruncatedTextSource(metadata, 0l, Math.min(getEndOffset(), 1000 * 1024l), this.delimiter);
	}
			...
			if (startOffset > 0) {
				checkState(channel instanceof SeekableByteChannel,
						"%s only supports reading from a SeekableByteChannel when given a start offset"
								+ " greater than 0.", TruncatedTextSource.class.getSimpleName());
