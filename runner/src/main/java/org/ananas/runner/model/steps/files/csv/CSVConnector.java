...
public class CSVConnector extends AbstractStepRunner implements Serializable {
	...
	public static SerializableFunction<String, Boolean> matchNonHeader(Schema schema, CSVFormat csvFormat) {
	...
	public CSVConnector(String stepId,
						CSVStepConfig config,
						Pipeline pipeline,
						boolean doSampling,
						boolean isTest) {
		super(StepType.Connector);
		CSVFormat format =
				CSVFormat.DEFAULT.withDelimiter(config.delimiter).withRecordSeparator(config.recordSeparator);
		CSVPaginator csvPaginator = new CSVPaginator(stepId, config);
		Schema inputschema = csvPaginator.getSchema();
		this.stepId = stepId;

		this.output =
				new BeamTextCSVCustomTable(this.errors, inputschema, config.url, format, config.hasHeader, doSampling,
						isTest).buildIOReader(pipeline);
