...
public class SchemaBasedRowConverter implements RowConverter, Serializable {
	...
	private Row convertMap(Map<String, Object> o, Schema rowSchema) {
		return IntStream.range(0, rowSchema.getFieldCount())
				.mapToObj(idx ->
						convert(o.get(rowSchema.getField(idx).getName()), rowSchema.getField(idx).getType(),
	...
	private List<Object> convertArray(Object value, Schema.FieldType collectionElementType,
									  String fieldName) {
		if (value == null) {
		...
		if (!(value instanceof List)) {
			this.errors.addError(ExceptionHandler.ErrorCode.GENERAL,
					String.format("For field name %s and array type expected List. Instead "
							+ " value was %s.", fieldName, value));
			return Collections.EMPTY_LIST;
		}
	...
	private Map<Object, Object> verifyMap(Object value, Schema.TypeName keyTypeName,
										  Schema.FieldType valueType, String fieldName) {
		if (value == null) {
			return Collections.EMPTY_MAP;
		}
		if (!(value instanceof Map)) {
			this.errors.addError(ExceptionHandler.ErrorCode.GENERAL, String.format(
		...
		for (Map.Entry<Object, Object> kv : valueMap.entrySet()) {
			verifiedMap.put(convertPrimitiveType(kv.getKey(), keyTypeName, fieldName),
		...
		if (!(value instanceof Row)) {
			...
			this.errors.addError(ExceptionHandler.ErrorCode.GENERAL,
					String.format("For field name %s expected Row type. "
							+ "Instead value was %s.", fieldName, value));
			return null;
		}
	...
	private Instant convertDateTime(Object value, String fieldName) {
		...
		if (value instanceof AbstractInstant) {
			...
			this.errors.addError(ExceptionHandler.ErrorCode.GENERAL,
					String.format("For field name %s and DATETIME type got unexpected value %s ",
							fieldName, value));
			return null;
		}
	}
