...
public class JdbcPaginator extends AbstractPaginator implements Paginator {
	...
	public SelectQuery outputQuery(boolean isLimit) {
		Query inputQuery = DSL.using(this.config.sqlDialect.JOOQdialect)
				.parser()
				.parseQuery(this.config.sql);
		String outputSQL = DSL.using(this.config.driver.JOOQdialect).render(inputQuery);
		SelectQuery q = (SelectQuery) DSL.using(this.config.driver.JOOQdialect)
				.parser().parseQuery(outputSQL);
		if (isLimit) {
	...
	public Schema autodetect() {
		...
		return JdbcSchemaDetecter.autodetect(this.config.driver, this.config.url, this.config.username,
				this.config.password, q.toString());
	}

	public static void main(String[] args) {
		SelectQuery inputQuery = (SelectQuery) DSL.using(SQLDialect.DERBY)
				.parser().parseQuery("select * from table_test2");
