...
public class JdbcSchemaDetecter implements Serializable {
	...
	public static Schema autodetect(JDBCDriver driver, String url, String username, String password, String sql) {
		//see https://github.com/pgjdbc/pgjdbc
		LOG.debug("Schema autodetect SQL {}", sql);
		return JDBCStatement.Execute(driver, url, username, password,
				(conn, statement) -> {
	...
	public static Schema extractSchema(JDBCDriver driver, ResultSet rs) throws SQLException {
		...
		for (int i = 1; i <= metadata.getColumnCount(); i++) {
			...
			String typeName =
					metadata.getColumnTypeName(i) != null ? metadata.getColumnTypeName(i).toLowerCase() : "text";
			builder.addNullableField(name, driver.getDefaultDataTypeLiteral(typeName) ==
												   null ? Schema.FieldType.STRING : driver.getDefaultDataTypeLiteral(
					typeName));
		}
	...
	public static Object autoCast(ResultSet resultSet, int idx, Schema schema) {
		...
		try {
			...
			LOG.warn("FETCH AUTOCAST WARNING : " + "idx= " + idx + " type: " + type + "  \n" + e.getMessage());
		}
		return null;
	}
