...
public class TruncatedCompressedSource extends FileBasedSource<String> {
	 ...
	 */
	private TruncatedCompressedSource(TruncatedTextSource sourceDelegate,
	...
	protected FileBasedSource<String> createForSubrangeOfFile(Metadata metadata, long start, long end) {
		return new TruncatedCompressedSource(
				(TruncatedTextSource) this.sourceDelegate.createForSubrangeOfFile(metadata, start, 1000 * 1024),
				this.channelFactory, metadata, this.sourceDelegate.getMinBundleSize(), start, 1000 * 1024);
	}
	...
	protected final FileBasedReader<String> createSingleFileReader(PipelineOptions options) {
		return new CompressedReader(this,
	...
	public void populateDisplayData(DisplayData.Builder builder) {
		...
		builder
				...
				.add(DisplayData.item("source", this.sourceDelegate.getClass())
		...
		if (this.channelFactory instanceof Enum) {
			...
			builder.add(DisplayData.item("compressionMode", ((Enum) this.channelFactory).name())
					.withLabel("Compression Mode"));
		} else {
			builder.add(DisplayData.item("compressionMode", this.channelFactory.getClass())
					.withLabel("Compression Mode"));
		}
	}
	...
	public static class CompressedReader extends FileBasedReader<String> {
		 ...
		 */
		CompressedReader(TruncatedCompressedSource source, TruncatedTextSource.TextBasedReader readerDelegate) {
		...
		protected final void startReading(ReadableByteChannel channel) throws IOException {
			...
			if (this.channelFactory == CompressionMode.AUTO) {
				...
				this.readerDelegate.startReading(this.channelFactory.createDecompressingChannel(
						channel));
			}
		}
