...
public class GradientTreeBoostHook extends ClassificationHook {
	...
	public GradientTreeBoostHook(String mode,
								 Pipeline pipeline,
	...
	protected MutableQuadruple<Schema, Iterable<Row>, String, Classifier<double[]>> trainTemplate(Attribute[] attributes,
		...
		GradientTreeBoost model =
				new GradientTreeBoost(attributes, x, y, ntrees);

		Schema schema = Schema.builder().addNullableField("dot", Schema.FieldType.STRING)
				.addNullableField("maxDepth", Schema.FieldType.INT32).build();

		List<Row> a = new ArrayList<>();
		for (int i = 0; i < model.getTrees().length; i++) {
			a.add(Row.withSchema(schema).addValue(model.getTrees()[i].dot()).addValue(
					model.getTrees()[i].maxDepth()).build());
		}
