...
public enum JDBCDriver implements Serializable {
	...
	MYSQL("mysql", "com.mysql.jdbc.Driver", MySQLDataTypes.BIGINT, MySQLDialect.of(), org.jooq.SQLDialect.MYSQL, true),
	POSTGRESQL("postgres", "org.postgresql.Driver", PostgresqlDataTypes.BIGINT, PostgresqlDialect.of(),
			org.jooq.SQLDialect.POSTGRES, true),
	DERBY("derby", "org.apache.derby.jdbc.EmbeddedDriver", DerbyDataTypes.BIGINT, DerbySQLDialect.of(),
			org.jooq.SQLDialect.DERBY, false),
	...
	public JDBCDriver getDriverByName(String name) {
		...
		throw new RuntimeException(String.format("%s is not supported yet. Please choose another DB name.", name));
	}
	...
	public void setParameter(int idx, Schema.FieldType type, PreparedStatement query, Object o) {
		try {
			...
			switch (type.getTypeName()) {
				...
				case DECIMAL:
					if (this == DERBY) {
						query.setBigDecimal(idx,
								o == null ? null : ((BigDecimal) o).setScale(2, BigDecimal.ROUND_DOWN));
					} else {
			...
			LOG.debug("setParameter AUTOCAST WARNING : " + "idx= " + idx + " type: " + type.getTypeName() + "  \n" +
					e.getMessage());
		}
	}
