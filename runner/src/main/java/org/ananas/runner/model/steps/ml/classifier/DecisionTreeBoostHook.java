...
public class DecisionTreeBoostHook extends ClassificationHook {
	...
	public DecisionTreeBoostHook(String mode,
								 Pipeline pipeline,
	...
	protected MutableQuadruple<Schema, Iterable<Row>, String, Classifier<double[]>> trainTemplate(Attribute[] attributes,
		...
		DecisionTree model =
				new DecisionTree(attributes, x, y, maxNodes);
		...
		a.add(Row.withSchema(schema).addValue(model.dot()).addValue(
				model.maxDepth()).build());
