...
public class JdbcConnector extends AbstractStepRunner implements StepRunner, Serializable {
	...
	public JdbcConnector(Pipeline pipeline,
						 String stepId,
		...
		PCollection<Row> p =
				pipeline.apply(JdbcIO.<Row>read().withDataSourceConfiguration(JdbcIO.DataSourceConfiguration.create(
	...
	public static JdbcIO.RowMapper<Row> rowMapper(Schema schema, ErrorHandler handler) {
		return r -> {
			try {
				return IntStream.range(0, schema.getFieldCount()).mapToObj(
						idx -> JdbcSchemaDetecter.autoCast(r, idx, schema)).collect(toRow(schema));
			} catch (Exception e) {
				handler.addError(e);
				return null;
			}
