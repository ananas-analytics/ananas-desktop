...
public class MongoDbIO {
	...
	public abstract static class Read extends PTransform<PBegin, PCollection<Document>> {
		 ...
		 * you specific with {@link org.apache.beam.sdk.io.mongodb.MongoDbIO.Read#withDatabase(String)}.</li>
		 ...
		 * instead you have to use {@link org.apache.beam.sdk.io.mongodb.MongoDbIO.Read#withKeepAlive(boolean)}. Same for the
		 * {@code MaxConnectionIdleTime} connection option via
		 * {@link org.apache.beam.sdk.io.mongodb.MongoDbIO.Read#withMaxConnectionIdleTime(int)}.
		 * </li>
		 * </ul>
		 */
		...
		public PCollection<Document> expand(PBegin input) {
			...
			return input.apply(org.apache.beam.sdk.io.Read.from(new MongoDbIO.BoundedMongoDbSource(this)));
		}
			...
			try (MongoClient mongoClient = new MongoClient(new MongoClientURI(this.spec.uri()))) {
				...
				if (this.spec.numSplits() > 0) {
					...
					long estimatedSizeBytes = getEstimatedSizeBytes(mongoClient,
							this.spec.database(), this.spec.collection());
				...
				splitVectorCommand.append("splitVector", this.spec.database() + "." + this.spec.collection());
