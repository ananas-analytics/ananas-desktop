...
public class JavascriptRowTransformer extends AbstractStepRunner implements StepRunner, Serializable {
	...
	public JavascriptRowTransformer(String stepId,
									StepRunner previous,
									String functionName,
									String sample,
									List<String> scripts) {
		...
		this.output = previous.getOutput()
				.apply("row To String",
						AsJsons.of(this.errors))
				.apply("javascript transform",
						new JavascriptTextTransform(functionName, scripts, this.errors))
				.apply("string to Row",
						new JsonStringBasedFlattenerReader(SchemaBasedRowConverter.of(outputSchema),
								this.errors)
				);
	 ...
	 */
	private Schema validate(Schema inputSchema, String functionName, String sample, List<String> scripts) {
		JavascriptRuntime javascriptRuntime = null;
		ObjectMapper customMapper = new ObjectMapper();
		try {
			javascriptRuntime =
					new JavascriptRuntime(functionName, scripts);
		...
		try {
			...
			for (int j = 0; j < jsons.size(); j++) {
				//infer types here
				JsonAutodetect.inferTypes(types, jsons.get(j),
