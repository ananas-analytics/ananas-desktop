 ...
 * For testing purpose, the pipelines has step that fetches data in memory and autodetectBson input and output schemas.
 */
public class StepBuilder {

	private static final Logger LOG = LoggerFactory.getLogger(StepBuilder.class);

	//private LinkedList<StepRunner> stepRunners;


	/*public StepRunner getLastStep(StepMatcher matcher) {
		for (int i = this.stepRunners.size() - 1; i == 0; i--) {
			StepRunner stepRunner = this.stepRunners.get(i);
			if (matcher.match(stepRunner)) {
				return stepRunner;
			}
		}
		return null;
	...
	public static StepRunner join(Step step,
								  StepRunner one,
								  StepRunner other) {
		String leftStepId =
				(String) step.config.get(StepConfig.JOIN_LEFT_STEPID);
		...
		Map<String, String> columnsMap =
				(Map) step.config.get(StepConfig.JOIN_MAP);
		//List<String> leftColumns = Arrays.asList("inventoryId");
		String joinType = (String) step.config.getOrDefault(StepConfig.JOIN_TYPE,
				JoinConnector.JoinType.LEFT_JOIN.name);
		List<String> leftColumns =
				(List) step.config.get(StepConfig.JOIN_LEFT_COLUMNS);
		List<String> rightColumns =
				(List) step.config.get(StepConfig.JOIN_RIGHT_COLUMNS);
		//List<String> rightColumns = Arrays.asList("inventoryId");
		return new JoinConnector(step.id,
				joinType,
				leftStep, rightStep, leftColumns,
				rightColumns,
				columnsMap);
	}

	public static StepRunner concat(Step step,
									StepRunner one,
									StepRunner other) {
		String leftStepId =
				(String) step.config.get(StepConfig.JOIN_LEFT_STEPID);
		...
		return new ConcatConnector(step.id,
				leftStep, rightStep);
	}


	public static StepRunner connector(Step step,
			...
			case "file":
				switch ((String) step.config.get(StepConfig.FORMAT)) {
					case "csv":
						...
						return new CSVConnector(step.id,
								csvConfig,
								context.getPipeline(), doSampling, isTest);
					case "text":
					...
					case "excel":
						...
						return new ExcelConnector(
								step.id,
								context.getPipeline(), excelConfig,
								doSampling,
								isTest);
					case "any":
					...
					default:
						throw new IllegalStateException("Unsupported files format '" +
								step.config.get(StepConfig.FORMAT) + "'");
				}
			case "kafka":
				switch ((String) step.config.get(StepConfig.FORMAT)) {
					case "json":
						return new KafkaConnector(
								...
								"test");
					default:
						throw new IllegalStateException("Unsupported Kafka message format '" +
								step.config.get(StepConfig.FORMAT) + "'");
				}
				/* testing events
    ...
    "vast.PlayerMute", "vast.PlayerPause", "vast.PlayerResume", "vast.PlayerRewind", "vast.PlayerSkip", "vast.PlayerUnmute"
			...
			default:
				throw new IllegalStateException(
						"Unsupported connector type '" + step.type + "'");
		}
	}


	public static StepRunner append(Step step,
									StepRunner previous,
									boolean isTest) {
		switch (step.type) {
	...
	public static StepRunner transformer(Step step,
										 StepRunner previous,
										 boolean isTest) {
		switch ((String) step.config.get(StepConfig.SUBTYPE)) {
			case "sql":
				return new SQLTransformer(step.id,
						(String) step.config.get(StepConfig.SQL),
						previous);
			case "js":
			...
			case "histogram":
				return new HistogramTransformer(step,
						previous);
			default:
				throw new IllegalStateException("Unsupported step type '" + step.config.get(StepConfig.SUBTYPE) + "'");
		}
	}

	public static StepRunner dataViewer(Step step, StepRunner previous, boolean isTest) {
		if (isTest) {
			StepRunner sqlQuery = new SQLTransformer(step.id,
					(String) step.config.get(StepConfig.DATAVIEW_SQL),
					previous);
	...
	public static StepRunner loader(Step step, StepRunner input, boolean isTest) {
		switch ((String) step.config.get(StepConfig.SUBTYPE)) {
			case "file":
				switch ((String) step.config.get(StepConfig.FORMAT)) {
					case "json":
						return new FileLoader(input, step.id, FileLoader.SupportedFormat.JSON,
								(String) step.config.get(StepConfig.PATH),
								(String) step.config.get(StepConfig.SHARD),
								(String) step.config.get(StepConfig.PREFIX), false, isTest);
					case "csv":
						return new FileLoader(input, step.id, FileLoader.SupportedFormat.CSV,
								(String) step.config.get(StepConfig.PATH),
								(String) step.config.get(StepConfig.SHARD),
								(String) step.config.get(StepConfig.PREFIX),
								(Boolean) step.config.getOrDefault(StepConfig.IS_HEADER, false), isTest);
					default:
						throw new IllegalStateException(
								"Unsupported step config subtype '" + step.config.get(StepConfig.FORMAT) + "'");
				}
			case "jdbc":
				JDBCDriver jdbcDriver = JDBCDriver.NONE.getDriverByName((String) step.config.get(StepConfig.JDBC_TYPE));
				...
				return new JdbcLoader(
						...
						input, isTest);
			default:
				throw new IllegalStateException("Unsupported subtype'" + step.config.get(StepConfig.SUBTYPE) + "'");
		}
	}

	public static StepRunner mlTransformer(Step step,
		...
		if (IsStepTrainingMode.of().filter(step)) {
			...
			return new MLModelPredictor(step, previous);//Note here the context is the previous step context
		}
	}
}
