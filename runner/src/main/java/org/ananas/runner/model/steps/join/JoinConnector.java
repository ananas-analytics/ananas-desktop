...
public class JoinConnector extends AbstractStepRunner implements StepRunner, Serializable {
	 ...
	 */
	public JoinConnector(String id,
						 String joinType,
						 StepRunner leftStep,
		...
		this.output = collectionTuple.apply(
				SqlTransform.query(
						"SELECT " + SQLProjection("TableLeft", leftColumns) + ", " +
								SQLProjection("TableRight", rightColumns) + " FROM TableLeft " + join.sqlExpression +
								" TableRight ON " +
								SQLJoin(columnsMap)));
	}
	...
	private String SQLProjection(String tableName, List<String> columns) {
		return Joiner.on(" , ").join(columns.stream().map(
				c -> tableName + "." + "`" + c + "`").iterator());
	}

	private String SQLJoin(Map<String, String> joinedColumns) {
		return Joiner.on(" AND ").join(joinedColumns.entrySet().stream().map(
				e -> String.format("CAST(TableLeft.`%s` as VARCHAR) = CAST(TableRight.`%s` as VARCHAR)", e.getKey(),
						e.getValue())).iterator());
	}
