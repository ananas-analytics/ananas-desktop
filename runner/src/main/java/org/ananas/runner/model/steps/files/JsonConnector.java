...
public class JsonConnector extends AbstractStepRunner implements Serializable {
	...
	public JsonConnector(Pipeline pipeline, String stepId, String url, boolean doSampling, boolean isTest) {
		super(StepType.Connector);
		Schema schema = autodetect(url);
		this.stepId = stepId;
		PCollection<String> p = pipeline.apply(isTest ? TruncatedTextIO.read().from(url) : TextIO.read().from(url));
		this.output = Sampler.sample(p, DEFAULT_LIMIT, (doSampling || isTest)).apply(
