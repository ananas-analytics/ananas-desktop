...
public abstract class RegressionLinearHook<T extends Regression<double[]>> extends MLHookTemplate {
  ...
  protected MutableTriple<Schema, Iterable<Row>, String> test(AttributeDataset dataset) {
    ...
    List<Row> a =
        Arrays.asList(Row.withSchema(schema).addValue(count).addValue(meanSquareError).build());

    return MutableTriple.of(schema, a, "Test completed");
  }

  /**
   * Train and serialize legacy
   *
   * @param dataset
  ...
  protected MutableTriple<Schema, Iterable<Row>, String> train(AttributeDataset dataset) {
    ...
    MutableQuadruple<Schema, Iterable<Row>, String, Regression<double[]>> tuple =
  ...
  /** Train and serialize legacy */
  protected abstract MutableQuadruple<Schema, Iterable<Row>, String, Regression<double[]>>
      trainTemplate(Attribute[] attributes, double[][] x, double[] y);

  protected void serialize(Regression<double[]> model, FeatureTransform standardizer) {
    SerializationUtils.serialize(
        RegressionWrapper.of(model, standardizer), getSerializedModelPath(), true);
  }
}
