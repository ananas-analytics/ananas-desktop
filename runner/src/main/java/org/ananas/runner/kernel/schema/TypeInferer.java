...
public class TypeInferer {
  ...
  static {
    ...
    COVARIANT_TYPES.put(
        Schema.FieldType.DOUBLE, Arrays.asList(Schema.FieldType.FLOAT, Schema.FieldType.STRING));
      ...
      if (Objects.equals(one.getRowSchema(), other.getRowSchema())) {
        ...
        for (Schema.Field otherField : other.getRowSchema().getFields()) {
          ...
          if (oneField == null) {
            ...
            b.addNullableField(
                oneField.getName(), mergeType(oneField.getType(), otherField.getType()));
          }
        }
  ...
  /**
   * Attempts to interpret the Object <tt>value</tt> as a representation of any type. If the attempt
   * is successful the type is returned.
   *
   * @param value
